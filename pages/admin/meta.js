...
const Meta = ({ data }) => {
  ...
  const formHandler = () => {
    if (testi) {
      ...
      fetch("/api/meta", requestOptions).then(
  ...
  const editFormHandler = () => {
    if (testiId) {
      const requestOptions = {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ id: testiId, name: testi, content: content }),
      };
      fetch("/api/meta", requestOptions).then(
        (response) => response.json(),
        refreshData()
  ...
  return (
    <Stack width={"100%"}>
      <Flex direction={"column"}>
          ...
          >
            ...
            <BreadcrumbItem isCurrentPage>
              <NextLink href="/admin/testimonials">
                <BreadcrumbLink>Meta Tags</BreadcrumbLink>
              </NextLink>
            </BreadcrumbItem>
          </Breadcrumb>
        </Box>
        <Box w="100%" p={4} bg="gray.100" borderRadius={"10px"} mt={10}>
          <Flex justifyContent={"space-between"}>
            <Box>
              <Heading fontSize={"2xl"}>Meta Tags</Heading>
            </Box>
            <Box>
          ...
          <Box maxWidth={"73vw"}>
            <TableContainer mt="10">
              <Table variant="striped" colorScheme="blackAlpha">
                <TableCaption>Meta Tags Table</TableCaption>
                <Thead>
                  <Tr>
                    <Th>SI.No</Th>
                    <Th>Name</Th>
      ...
      <Modal isOpen={isOpen} onClose={onClose}>
        <ModalOverlay />
        <ModalContent>
          <ModalHeader>Add New Meta</ModalHeader>
          <ModalCloseButton />
          <ModalBody>
            <FormControl>
              <FormLabel htmlFor="name">Meta page url</FormLabel>
              <Input
                id="name"
                type="text"
      ...
      <Modal isOpen={isDeleteOpen} onClose={onDeleteClose}>
        <ModalOverlay />
        <ModalContent>
          <ModalHeader>Delete Meta</ModalHeader>
          <ModalCloseButton />
          <ModalBody>
            <Text>
...
export async function getServerSideProps() {
  ...
  let data = await db.collection("meta").find({}).toArray();
...
Meta.getLayout = function getLayout(Meta) {
  return <AdminLayout>{Meta}</AdminLayout>;
};
